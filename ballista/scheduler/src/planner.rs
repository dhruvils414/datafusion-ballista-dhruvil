...
fn create_unresolved_shuffle(
    ...
    Arc::new(UnresolvedShuffleExec::new(
        ...
        shuffle_writer.properties().partitioning.partition_count(),
    ))
}
    ...
    for child in stage.children() {
        if let Some(shuffle_reader) = child.as_any().downcast_ref::<ShuffleReaderExec>() {
            let output_partition_count =
                shuffle_reader.properties().partitioning.partition_count();
...
",
        ...
        */
        ...
        assert_eq!(2, input.properties().partitioning.partition_count());
        assert_eq!(
            2,
            stages[2]
                .shuffle_output_partitioning()
